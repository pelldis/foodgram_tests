...
class TestAuth:
    ...
    def test_create_user__invalid_request_data(self, client, user):
        url = self.url_create
        response = client.post(url)
        ...
        fields_invalid = ['email', 'username', 'password', 'first_name', 'last_name']
        ...
        fields = ['email', 'email', 'username', 'first_name', 'last_name', 'password']
        for data, field in zip(invalid_data, fields):
            response = client.post(url, data=data)
            print(response.json(), response.status_code)
            assert response.status_code == HTTPStatus.BAD_REQUEST, (
                'Убедитесь, что POST-запрос с некорректными данными, '
                f'отправленный к`{url}`, возвращает ответ со статусом 400.'
            )
            assert field in response.json(), (
                'Убедитесь, что в ответе на POST-запрос с некорректными '
                f'данными, отправленный к `{url}`, содержится поле `{field}` '
